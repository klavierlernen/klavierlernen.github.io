  ...
  if (metronomeOn) {
    ...
    if (!existingInput) {
      ...
      customModeToggle.addEventListener("click", () => {
        ...
        if (!modal.querySelector("#customModeContent")) {
          const container = document.createElement("div");
          container.id = "customModeContent";
          container.style.marginTop = "10px";
          modal.appendChild(container);

          populateCustomNotes(); // Container existiert nun sicher
        }
      });
  ...
  if (!existingInput) {
    ...
    input.addEventListener("blur", () => {
      ...
      blobToggle.addEventListener("click", () => {
        ...
        resetArticulationMode();
        blobMode = !blobMode;
        blobToggle.style.opacity = blobMode ? "1" : "0.5";
        drawSeries();
      });
      ...
      const carouselButtons = [pauseButton, blobToggle, kidsModeToggle, darkModeToggle, moveNotationToggle, wizardToggle, staccatoToggle];
...
const generateSeries = () => {
  ...
  if (
    ...
    window.customModeSettings.selectedNotes &&
    window.customModeSettings.selectedNotes.length > 0 &&
    window.customModeSettings.clef
  ) {
    // Benutzerdefinierter Modus aktiv
    seriesClef = window.customModeSettings.clef;
    currentRange = window.customModeSettings.range;
    const customNotes = window.customModeSettings.selectedNotes;

    chooseNote = () => {
      const randomIndex = Math.floor(Math.random() * customNotes.length);
      const chosenNoteRaw = customNotes[randomIndex];
      // Remove apostrophes for base note (e.g. c', c'')
      ...
      if (["F", "G"].includes(currentRange)) {
        if (currentRange === "F" && baseNote === "bb") {
          accidental = "b";
        } else if (currentRange === "F" && baseNote.includes("b") && baseNote !== "b") {
          accidental = baseNote;
        } else if (currentRange === "G" && (baseNote.includes("#") || (baseNote.includes("b") && baseNote !== "b"))) {
          accidental = baseNote;
        }
      }

      return { note: baseNote, octave: noteOctave, clef: seriesClef, color: "black", accidental: accidental };
    };
...
const drawSeries = () => {
  ...
  const staveNotes = currentSeriesSingle.map(item => {
      let noteName = item.note.toLowerCase().replace(/♯/g, "#").replace(/♭/g, "b").replace(/^h$/, "b");
...
const handleMIDIMessage = (status, data1, data2) => {
  ...
  if (
    ...
    articulationMode === "staccato"
  ) {
    if (activeNotes[data1]) {
      const noteData = activeNotes[data1];
      checkArticulation(noteData.timestamp, Date.now(), noteData.velocity);
  ...
  if ((status & 0xf0) === 0x90 && data2 > 0) {
    ...
    if (articulationMode === "staccato") {
      if (!activeNotes[data1]) {
        activeNotes[data1] = { timestamp: currentTime, velocity: data2 };
      }
      return;
    }

    // Nur im Nicht-Artikulationsmodus fortfahren
  ...
  // --- checkArticulation Funktion ---
  function checkArticulation(startTime, endTime, velocity) {
    // Nur noch Staccato prüfen, Legato entfällt
    let correct = false;
    if (articulationMode === "staccato") {
      correct = (endTime - startTime) < 350 && velocity > 30;
    }

  // Hilfsfunktion: Staccato-Modus deaktivieren
  function deactivateStaccatoMode() {
    articulationMode = null;
    const notation = document.getElementById("notation");
    if (notation) notation.style.display = "block";
    const clefTitle = document.getElementById("clefTitle");
    if (clefTitle) clefTitle.textContent = currentRange + "-Lage";
    const staccatoToggle = document.getElementById("staccatoToggle");
    if (staccatoToggle) staccatoToggle.style.opacity = "0.5";
  }
    // Feedback
    const clefTitle = document.getElementById("clefTitle");
    if (correct) {
      clefTitle.textContent = "Richtig!";
      setTimeout(() => {
        document.getElementById("clefTitle").textContent = "🥁 Staccato";
      }, 700);
    } else {
      clefTitle.textContent = "Zu lang! Kürzer spielen.";
    }
  }
  ...
  if (!localStorage.getItem('tutorialCompleted')) {
    startTutorialSequence();
        ...
        function updateAccuracyMarker(deviationRatio) {
          ...
          const accuracy = Math.max(0, Math.min(100, 100 - Math.abs(deviationRatio * 100)));
          display.textContent = `${Math.round(accuracy)}%`;
          display.style.display = "block";

          clearTimeout(display._hideTimeout);
          display._hideTimeout = setTimeout(() => {
            display.style.display = "none";
          }, 1000);
        }
        
        function checkArticulation(startTime, endTime, velocity) {
          const expected = 300; // Ziel-Staccato-Zeit in ms
          const actual = endTime - startTime;
          const deviation = ((expected - actual) / expected) * 2;

          const correct = actual < 350 && velocity > 30;
          updateAccuracyMarker(deviation);

          const clefTitle = document.getElementById("clefTitle");
          if (clefTitle) {
            clefTitle.textContent = correct ? "Richtig!" : "Zu lang!";
          }
        }
          ...
          if (saveBtn) {
              saveBtn.addEventListener('click', () => {
                ...
                window.customModeSettings = {
                  clef: selectedClef,
                  range: selectedRange,
                  selectedNotes: selected.map(n => n.replace(/'/g, "").toLowerCase()) // Achtung: ' entfernen
                };
